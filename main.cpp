...
{
    out << "PrivilegedBridge 0.6 - local allowlisted system-action bridge\n\n"
        << "Usage:\n"
        ...
        << "Reference for Mistral and operators: " << kReferenceUrl << "\n\n"
        << "No GUI, network listener, or arbitrary shell interface is provided.\n"
        << "Privileged actions use pkexec/polkit; read-only and build actions run as the current user.\n";
}
}
...
int main(int argc, char *argv[])
{
    ...
    if (args.at(1) == "--reference" || args.at(1) == "--reference-url") {
        out << kReferenceUrl << '\n';
        return 0;
    }
