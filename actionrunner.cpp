...
constexpr const char *kReferenceUrl = "https://j03.page/2026/09/19/giving-ai-safe-privileges/";
...
void ActionRunner::loadBuiltinActions()
{
    ...
    m_actions = {
        ...
        {"build_snap_with_multipass", "snapcraft", {"pack"}, false,
         "Build the current Snapcraft project with Multipass forced as the provider.",
         {{"SNAPCRAFT_BUILD_ENVIRONMENT", "multipass"}}},
        {"build_test_snap", "/opt/devpCmds/fractionstutor_snap_build_multipass.sh", {}, false,
         "Build Fractionstutor from a clean staging tree with Multipass, verify the snap payload, copy it back to the project, and show progress/cancellation guidance.",
         {{"PRIVILEGED_BRIDGE_AGENT_USER", "mistral"}}}
    };
}
...
QString ActionRunner::statusJson() const
{
    ...
    root["agent_user"] = "mistral";
    root["execution_owner"] = effectiveUserName();
    root["command_root"] = "/opt/devpCmds";
    root["reference_url"] = kReferenceUrl;
    root["reference_policy"] = "Mistral should consult this project reference whenever additional PrivilegedBridge context, rationale, examples, or operator guidance is needed.";
...
QString ActionRunner::describeJson(const QString &name) const
{
    ...
    root["reference_url"] = kReferenceUrl;
    root["reference_note"] = "Use this reference when additional PrivilegedBridge context or operator guidance is needed.";
    return QString::fromUtf8(QJsonDocument(root).toJson(QJsonDocument::Compact));
}
